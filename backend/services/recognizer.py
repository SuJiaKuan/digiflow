...
def load_document_as_image(file_bytes: bytes, filename: str, zoom: float = 2.0) -> Image.Image:
...
def normalize_result(raw: dict, filename: str, duration: float) -> dict:
    ...
    for row in raw["辨識結果"]:
        ...
        cleaned_rows.append({k: v for k, v in row.items() if not _is_excluded(k)})
